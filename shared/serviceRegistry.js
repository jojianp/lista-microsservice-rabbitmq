...
class FileBasedServiceRegistry {
    ...
    // Cleanup na saída do processo - VERSAO SIMPLES
    cleanup() {
        const services = this.readRegistry();
        const currentPid = process.pid;
        let changed = false;

        // Remove TODOS os serviços, não só do PID atual
        Object.keys(services).forEach(name => {
            delete services[name];
            changed = true;
            console.log(`Removendo serviço ${name} do registry`);
        });
...
// Limpa o registry completamente quando o gateway é encerrado
process.on('exit', () => {
    const services = registry.readRegistry();
    if (Object.keys(services).length > 0) {
        console.log('Limpando todos os serviços do registry...');
